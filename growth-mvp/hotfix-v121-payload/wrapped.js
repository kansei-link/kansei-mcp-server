...
import { aggregateMonth, availableMonths, loadSessions } from "../usage/aggregate.js";
...
const DEFAULT_API_BASE = process.env.KANSEI_ENDPOINT_BASE || "https://kansei-link-mcp-production.up.railway.app";
...
function parseArgs(argv) {
    const args = { month: "", lang: "ja", json: false, share: false, help: false };
    for (let i = 0; i < argv.length; i++) {
        const a = argv[i];
        if (a === "--month")
            args.month = argv[++i] ?? "";
        else if (a === "--lang")
            args.lang = argv[++i] === "en" ? "en" : "ja";
        else if (a === "--json")
            args.json = true;
        else if (a === "--share")
            args.share = true;
...
function dominantModelRate(stats) {
    let best = "";
    let bestTokens = -1;
...
Requires the usage hook to be installed first:
  npx -y -p @kansei-link/mcp-server kansei-link-install-hooks`);
    ...
    if (args.json) {
        console.log(JSON.stringify({ ...stats, percentile_top: percentileTop, population, saved_usd_estimated: savedUsd }, null, 2));
        return;
    }
    // ── terminal report ──
    const L = ja
    ...
    lines.push(`  ${L.saved}: ${fmt(stats.saved_tokens_estimated)}  (≈ $${savedUsd.toFixed(2)}, ${costNote})`);
    ...
    if (stats.error_failed_calls > 0) {
        ...
        if (worst.length > 0)
            lines.push(`  ${L.worstTools}: ${worst.join(", ")}`);
    }
    ...
    writeFileSync(cardPath, renderCardHtml(stats, {
        lang: args.lang,
        percentileTop,
        population,
        savedUsd,
