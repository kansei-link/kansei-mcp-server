...
const DEFAULT_ENDPOINT = process.env.KANSEI_ENDPOINT ||
...
export function buildHookPayload(serviceId, success, toolName, errorType) {
    return {
...
function guessServiceId(input, serverName) {
    ...
    for (const k of candidates) {
        const v = obj[k];
        if (typeof v === "string" && SERVICE_SLUG_RE.test(v))
            return v;
    }
    return serverName;
}
...
function classifyError(payload) {
    ...
    if (!isError && !text.includes("error") && !text.includes("fail"))
        return null;
    if (text.includes("auth") || text.includes("401") || text.includes("unauthorized"))
        return "auth_error";
    if (text.includes("expired") || text.includes("refresh"))
        return "auth_expired";
    ...
    if (text.includes("not found") || text.includes("404"))
        return "not_found";
    if (text.includes("invalid") || text.includes("400"))
...
//# sourceMappingURL=report-hook.js.map