...
```bash
npx -y -p @kansei-link/mcp-server kansei-link-wrapped            # current month (JA)
npx -y -p @kansei-link/mcp-server kansei-link-wrapped --lang en  # English
npx -y -p @kansei-link/mcp-server kansei-link-wrapped --share    # opt-in: get your rank
```
...
```bash
npx -y -p @kansei-link/mcp-server kansei-link-install-skill
```
...
Live Updates consent (`npx -y -p @kansei-link/mcp-server kansei-link-live-updates --enable`) →
...
```json
{
  "hooks": {
    "PostToolUse": [{
      "matcher": "mcp__.*",
      "hooks": [{ "type": "command", "command": "npx -y -p @kansei-link/mcp-server kansei-link-report-hook" }]
    }]
  }
}
```
...
1. Verify the skill was installed:
   ...
   ```
   If absent, run `npx -y -p @kansei-link/mcp-server kansei-link-install-skill`.
