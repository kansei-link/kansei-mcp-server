...
<h2>なぜ動かなかったのか</h2>
<p>後から引用元を取得して分かりました。狙ったクエリで実際に読まれていたのは、<strong>他社のページ</strong>でした。自社の記事は到達可能な状態（sitemapにあり、公開されており、内部リンクもある）でしたが、<strong>${probeCites}件の引用のうち自社は${probeOurs}件</strong>です。</p>
...
<h2>ここから何をするか</h2>
<ol>
<li><strong>まず読める状態にする。</strong> これは前提で、飛ばせません。無料診断で確認できます</li>
<li><strong>買い手の言葉で書く。</strong> 自社の用語ではなく、相手が実際に打ち込む言い回しで</li>
<li><strong>読まれている場所に出す。</strong> 自社サイトだけでは、引用集合に入らないことがあります</li>
<li><strong>前後を測る。</strong> 測らないと、効いたかどうかが分かりません</li>
</ol>
...
<p class="note">この測定は自社が対象で、AIの回答に何が含まれたかを数えたものです。各社の品質評価ではありません。生成AIの回答は実行ごとに揺れるため、1回の結果で断定しないでください。</p>`,
  ...
  },
  {
    ...
    title: 'AI検索対策、何から始めるか — 測ってから直した記録',
    sub: `${DATE}・一次調査／施策の前後を同じ問いで測った`,
    desc: 'LLMO・AEO対策は何から始めるか。自社で施策の前後を測ったところ、順番を間違えると効果が分からなくなることが分かりました。測定を先に置く理由を、実測の記録から説明します。',
    stats: [
      [`0 / ${v1.cells}`, '施策後の自社言及数'],
      [`${probeCites}`, '狙ったクエリで測った引用元の数'],
    ],
    body: `
<p class="lead">AI検索対策で最初にやるべきことは、施策ではなく<strong>測定</strong>でした。順番を逆にすると、何が効いたのか分からなくなります。自社で試して、実際にそうなりました。</p>

<h2>測定を先に置く理由</h2>
<p>私たちは施策を先に打ちました。構造化データ、llms.txt、想定質問の掲載。そのあとで測ったところ、<strong>言及は0のまま</strong>でした。ここで困ったのは、次のどれなのかが分からないことです。</p>
<ul>
<li>施策が間違っていたのか</li>
<li>施策は正しいが、効果が出るまで時間がかかるのか</li>
<li>そもそも施策前から0で、変化しようがなかったのか</li>
</ul>
<p>基準点を取っていなかったので、3つ目すら確かめられませんでした。<strong>先に測っておけば、この迷いは生まれません。</strong></p>

<h2>順番</h2>
<ol>
<li><strong>買い手の言い回しを10問ほど集める。</strong> 自社の用語ではなく、相手が打ち込む言葉で</li>
<li><strong>基準点を測る。</strong> 複数エンジンに投げ、固有名の出現と引用元を記録する</li>
<li><strong>読める状態にする。</strong> AIがサイトを読めるかを確認する（無料診断で分かります）</li>
<li><strong>読まれている場所を調べる。</strong> 引用元を見れば、AIがどこを読んでいるか分かります</li>
<li><strong>もう一度測る。</strong> 同じ問いで。動いたかどうかは、これでしか分かりません</li>
</ol>

<h2>引用元を記録すると、打ち手が具体的になります</h2>
<p>狙ったクエリで${probeCites}件の引用元を取得したところ、読まれていたのは大手メディアではなく、中小の事業者のブログでした。60ドメインに分散していて、権威が集中していません。</p>
<p>これは<strong>入り込む余地があるという意味でもあります</strong>。誰が読まれているかが分かれば、そこに合わせて書けます。分からないまま書くと、当て推量になります。</p>
