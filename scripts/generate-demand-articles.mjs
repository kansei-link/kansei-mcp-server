...
// まとめ記事を書く側もAIに下書きさせている前提に立つと、数字だけ抜かれると社名が消える。
// 出典を文の中に埋めた一文をコピーできる形で置き、抜かれても名前が一緒に運ばれるようにする。
const citeBlock = text => `<div class="cite"><div class="c-h">この記事を引用する場合（そのままお使いください）</div><blockquote>${esc(text)}</blockquote></div>`;
...
const section = sec => {
  ...
  for (const b of sec.blocks) {
    ...
    else if (b.cite) parts.push(citeBlock(b.cite));
