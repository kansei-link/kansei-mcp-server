...
export function register(server: McpServer, db: Database.Database): void {
  server.tool(
    ...
    async ({ service_id, snapshot_date }) => {
      const result = takeSnapshot(db, { service_id, snapshot_date });
      return {
        content: [
          {
            type: "text" as const,
...
export function takeSnapshot(
  ...
  opts: { service_id?: string; snapshot_date?: string }
): object {
  const date =
    opts.snapshot_date ||
  ...
  // Get target services
  const services = opts.service_id
    ? db.prepare("SELECT id, category, trust_score FROM services WHERE id = ?").all(opts.service_id) as any[]
    : db.prepare("SELECT id, category, trust_score FROM services").all() as any[];

  if (services.length === 0) {
    return { error: "no_services_found", service_id: opts.service_id };
  }
