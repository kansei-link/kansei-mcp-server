...
## 2026-07-06 — Full 11k Endpoint Liveness Sweep + archived Semantics Hardening
CONTEXT: First full liveness sweep of all 11,081 registered services (agent-driven, 183s: GitHub GraphQL batch for 6.3k repos, MCP POST-initialize for ~4.2k hosted endpoints, npm registry for npx entries). Findings: 76.2% strict liveness; 43% of GET-404 hosted endpoints were actually ALIVE on a proper POST initialize (Streamable HTTP servers legitimately 404/405 a plain GET); all 191 Smithery entries genuinely delisted (control test: live Smithery returns 401 without a key, delisted returns 404 "Server not found"); the npx endpoints of flagship curated entries (freee, slack, stripe-jp, asana...) were almost all fabricated package names. Separately, the concurrent daily crawl (#34) un-archived 23 rows mid-sweep because refresh.ts only considered GitHub-repo-archived state, and search never read the archived column at all.
DECISION: (1) DB: archived=1 + trust_score=0 + axr NULL for 1,154 endpoint-dead services, with before-values recorded in service_changelog 'deprecated' entries (reversible); 379 endpoints corrected (306 GitHub renames, 25 /sse→/mcp-style moves, 48 npx fixes — every fix registry- or handshake-verified; 3 fabricated names NULLed: sansan/salesgo are contract-gated, telegram-bot has no trustworthy impl). (2) Code: refresh.ts never auto-un-archives (only ever sets 1); all 5 search paths filter COALESCE(archived,0)=0; generate-aeo-article.ts excludes archived from ranking pool + readiness counts; recipes double-JSON-encoding fixed in seed.ts + 200 rows repaired (root cause of lookup-combinations "steps.filter is not a function" crash); service-stats-seed.json (1,019 rows, scripts/export-stats-seed.mjs) backfills empty service_stats on fresh deploys so insights stop contradicting voices; health-probe.ts refactored to exported runHealthProbe with 404/410→archive semantics and wired into run.ts as weekly stage 10/11 (Sundays, options.probe to force). (3) Proposals queue: all 17 pending_updates were already reviewed (Apr/Jun); SmartHR per_page pagination trap folded into agent_tips; test/praise feedback acknowledged.
REASON: Dead endpoints actively harm agents — npx-installing a fabricated package name is the worst case (the DB's own anti-guessing principle applied to itself). GET-based liveness is provably wrong for MCP (43% false positives), so POST initialize is the only honest probe, and the weekly probe stage keeps liveness decay visible instead of silently rotting.
REVERSIBILITY: DB archive fully reversible via changelog before-values + kansei-link.db.backup-20260706-pre-archive. Code changes: High (filters/flags). Probe cadence: options.probe.
COMMITS: (this session)
OPEN: (1) production Railway deploy of new dist + seed files (local DB is authoritative for the crawler but Railway volume has its own copy); (2) pSEO pages for archived services still live — noindex or prune pass; (3) product backlog from open feedback: compare_services tool (#6), AXR AAA with zero usage flagged inconsistent (#11), no add-service proposal path (#12), agent_voice rejects kansei-link self-reports (#13); (4) resurrection re-checks for archived rows (manual re-sweep; scripts in session scratchpad mcp-sweep/).
